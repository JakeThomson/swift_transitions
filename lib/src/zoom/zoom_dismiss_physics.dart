...
/// morph (`SheetMorphGeometry`, PR #223 and its follow-up), measured against
/// a cursor-tracked native capture to an RMS scale error of 0.006, ported
/// here by the same author. Travel is measured in *card heights* — for a
/// zoomed page the card is the page — because one card-relative gain
/// explained panels of different heights in that fit. See design.md section
/// 3.4, including the open question of what iOS treats as the card height
...
class ZoomDismissPhysics {
  ...
  /// Scale lost per card height of damped travel. Below 1.0 the fall always
  /// outruns the shrink, which is what keeps the card's bottom edge from
  /// lifting into view when the shrink pivots on the grabbed point.
  ...
  /// The spring the sideways offset returns home on after a release.
  final SpringDescription returnSpring;

  /// The card scale below which a release dismisses.
  final double dismissThreshold;

  /// The release speed, in logical pixels per second away from the identity
  /// state, that dismisses regardless of the card's scale.
